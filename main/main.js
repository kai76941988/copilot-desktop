...
function appendEnableFeatures (features) {
  var existing = app.commandLine.getSwitchValue('enable-features')
  if (existing) {
    app.commandLine.appendSwitch('enable-features', existing + ',' + features)
  } else {
    app.commandLine.appendSwitch('enable-features', features)
  }
}

var gpuAccelerationEnabled = settings.get('gpuAccelerationEnabled')
if (gpuAccelerationEnabled !== false) {
  app.commandLine.appendSwitch('ignore-gpu-blocklist')
  app.commandLine.appendSwitch('enable-gpu-rasterization')
  app.commandLine.appendSwitch('enable-zero-copy')
  appendEnableFeatures('CanvasOopRasterization')
}

if (settings.get('lowMemoryMode') === true) {
...
ipc.on('memory-capture', function (e, payload) {
  const win = windows.windowFromContents(e.sender) || windows.getCurrent()
  if (!win) return
  getWindowWebContents(win).send('memory-capture', payload)
})
...
let memoryHubWindow = null

function openMemoryHub () {
  if (memoryHubWindow && !memoryHubWindow.isDestroyed()) {
    memoryHubWindow.show()
    memoryHubWindow.focus()
    return
  }

  memoryHubWindow = new BrowserWindow({
    width: 1100,
    height: 820,
    show: true,
    webPreferences: {
  ...
  memoryHubWindow.loadURL('min://app/pages/memory/index.html')
}

ipc.on('open-memory-hub', function () {
  openMemoryHub()
})

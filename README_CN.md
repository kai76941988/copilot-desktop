...
- 🔗 **MCP 支持**：内置 MCP 服务器，支持 AI 助手直接控制浏览器
...
本应用内置 MCP 服务器，允许 AI 助手（如 Claude、Cursor、Windsurf）直接控制和操作浏览器。

### 配置 MCP

应用启动后，MCP 服务默认运行在 `http://127.0.0.1:3000`。您可以在 `pake.json` 中修改配置：
...
**Cursor / Windsurf**:
在设置中添加 MCP 服务器：`http://127.0.0.1:3000/sse`
...
| 资源 URI | 说明 |
|----------|------|
| `browser://current-url` | 当前页面 URL |
| `browser://title` | 页面标题 |
| `browser://content` | 页面 HTML 内容 |
| `browser://text` | 页面可见文本 |
| `browser://window-state` | 窗口状态 |
...
```
请帮我：
1. 导航到 https://copilot.microsoft.com
2. 等待搜索框出现
3. 输入"今天的新闻"
4. 截图并展示给我
```

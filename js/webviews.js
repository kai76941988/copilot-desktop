...
function refreshPerfSettings () {
  var lowMemorySetting = settings.get('lowMemoryMode')
  perfSettings.lowMemoryMode = (lowMemorySetting === true) || (lowMemorySetting === undefined && isLowMemoryDevice())

  var longArticleSetting = settings.get('longArticleMode')
  perfSettings.longArticleMode = (longArticleSetting !== false)
}

function buildPerfSettings () {
  return {
    lowMemoryMode: perfSettings.lowMemoryMode,
    longArticleMode: perfSettings.longArticleMode
  }
}

function broadcastPerfSettings () {
  var payload = buildPerfSettings()
  tasks.forEach(function (task) {
    task.tabs.forEach(function (tab) {
      if (webviews.hasViewForTab(tab.id)) {
        try {
          webviews.callAsync(tab.id, 'send', ['perfSettings', payload])
        } catch (e) {}
      }
    })
  })
...
function updateCaptureInterval () {
  ...
  var interval = perfSettings.lowMemoryMode ? 60000 : 15000
...
function captureCurrentTab (options) {
  ...
  var scale = perfSettings.lowMemoryMode ? 20 : 10
  ipc.send('getCapture', {
...
webviews.bindIPC('getPerfSettings', function (tabId, args) {
  webviews.callAsync(tabId, 'send', ['perfSettings', buildPerfSettings()])
})

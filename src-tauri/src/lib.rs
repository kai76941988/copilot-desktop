...
use app::{
    invoke::{
        ...
        memory_set_message_tags, memory_auto_tag_messages, memory_list_tags,
...
pub fn run_app() {
    ...
    app_builder
        .invoke_handler(tauri::generate_handler![
            ...
            memory_set_message_tags,
            memory_auto_tag_messages,
            memory_list_tags,
            memory_export_project,
        ])
        .setup(move |app| {
            ...
            let mcp_config = pake_config.mcp.clone();
            if mcp_config.enabled {
                let app_handle = app.app_handle().clone();
                std::thread::spawn(move || {
                    let rt = tokio::runtime::Runtime::new()
                        .expect("Failed to create Tokio runtime for MCP");
                    rt.block_on(app::mcp::start_mcp_server(app_handle, mcp_config));
                });
            }
